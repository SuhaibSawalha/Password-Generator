...
function changeLength(val) {
  ...
  const css = `#range::-webkit-slider-runnable-track {background: linear-gradient(to right,  #5BC08E 0%, #5BC08E ${
    (val * 100) / 15
  }%, rgb(29, 29, 29) ${(val * 100) / 15}%, rgb(29, 29, 29) 100%)}`;
  let style = document.getElementById("thumbColoring");
  if (style) {
    style.innerHTML = css;
    return;
  }
...
function Generate() {
  ...
  for (let i = 0; i < size; ++i) {
    randomValues.push(
      i < components.length
        ? getRandom(values[components[i]])
        : getRandom(values[getRandom(components)])
    );
  }
  for (let i = 0; i < size; ++i) {
    ...
    password += val;
    randomValues.splice(
      randomValues.findIndex((x) => x == val),
      1
    );
  }
...
function setStrength(index) {
  document.getElementById(
    "strength"
  ).innerHTML = `<h4 style="color: ${colors[index]}">${strengths[index]}</h4>`;
